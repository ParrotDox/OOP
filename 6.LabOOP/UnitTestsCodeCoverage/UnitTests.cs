...
namespace UnitTestsCodeCoverage
{
    public class UnitTests
    {
        ...
        public void Person_GetHashCode()
        {
            ...
            Person personClone = (Person)person.Clone();
            //Act
            bool areEqualHashes = person.GetHashCode() == personClone.GetHashCode();
            //Assert
            Assert.Equal(areEqualHashes, true);
        }
        ...
        [Fact]
        public void Employee_GetHashCode()
        {
            //Arrange
            Employee employee = new Employee("Name", 25, "Residence", 123, 8, 30005);
            Employee employeeClone = (Employee)employee.Clone();
            //Act
            bool areEqualHashes = employee.GetHashCode() == employeeClone.GetHashCode();
            //Assert
            Assert.Equal(areEqualHashes, true);
        }
        ...
        //SortByObjects
        [Fact]
        public void SortByObjects_Compare()
        {
            //Arrange
            bool flag = true;
            int prevAge = -1;
            ...
            for (int item = 0; item < mainArray.Count; ++item) 
            {
                if (((Person)mainArray[item]).__Age__ < prevAge)
                    flag = false;
                else 
                {
                    prevAge = ((Person)mainArray[item]).__Age__;
                }
            }
            //Assert
            Assert.Equal(true, flag);
        ...
        [Fact]
        public void Constructor_TestCollections() 
        {
            //Arrange
            ...
            Assert.NotNull(collections.personList);
            Assert.NotNull(collections.dictPE);
            Assert.NotNull(collections.dictSE);
        }
        [Fact]
        public void TestCollections_Add() 
        {
            //Arrange
            TestCollections collections = new TestCollections();
            //Act
            int prevCount = collections.personList.Count;
            Employee employee = new Employee();
            employee.RandomInit();
            collections.Add(employee);
            int afterCount = collections.personList.Count;
            //Assert
            Assert.Equal(prevCount+1, afterCount);
        }
        [Fact]
        public void TestCollections_Remove()
        {
            //Arrange
            TestCollections collections = new TestCollections();
            //Act
            ...
            employee.RandomInit();
            collections.Add(employee);
            collections.Remove(employee);
            int afterCount = collections.personList.Count;
            //Assert
            Assert.Equal(prevCount, afterCount);
        }
        [Fact]
        public void TestCollections_TestPerformance()
        {
            //Arrange
            TestCollections collections = new TestCollections();
            //Act
            string log = collections.TestPerformance();
            //Assert
            Assert.NotNull(log);
        }
    }
